    ...
    public class UserDAL
    {
        ...
        public bool UpdateUser(User user, DatabaseEntities de)
        {
            try
            {
                ...
                if (user.IsActive == 0)
                {
                    List<UserCourse> courseList = new UserCourseDAL().GetActiveUserCourseList(de).Where(x => x.UserId == user.Id).ToList();
                    foreach (UserCourse c in courseList)
                    {
                        c.IsActive = 0;
                        new UserCourseDAL().UpdateUserCourse(c, de);
                    }
                }
